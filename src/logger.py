...
class ImageLogger(Callback):
    ...
    def log_local_rgb(self, split, images, global_step, current_epoch, batch_idx, sample_tokens=None):
        """
        Log images as RGB masks, combining multiple masks into a single image.
        Args:
            sample_tokens: list of nuScenes sample tokens for this batch
        """
        root = os.path.join(self.save_dir, "image_log_" + split)
        ...
        # Use first sample token in batch for filename if available
        if sample_tokens is not None and len(sample_tokens) > 0:
            token_str = sample_tokens[0]  # Use first 8 chars of token for brevity
            filename = "rgb_e-{:03}_b-{:04}_{}.png".format(current_epoch, batch_idx, token_str)
        else:
            filename = "rgb_e-{:03}_b-{:04}.png".format(current_epoch, batch_idx)
        path = os.path.join(root, filename)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            # Extract sample_tokens from batch (batch is a tuple: imgs, rots, trans, intrins, post_rots, post_trans, bev_seg_gt, sample_token)
            if isinstance(batch, (list, tuple)) and len(batch) >= 8:
                sample_tokens = batch[7]  # sample_token is the 8th element
            else:
                sample_tokens = batch.get('sample_token', None) if isinstance(batch, dict) else None
            
            if self.image_style == 'video_frame':
                # Convert each image to a video frame
                self.log_frame(split, images, batch_idx, sample_tokens)
            elif hasattr(pl_module, "control_key"):   # for cldm
                self.log_local_rgb(split, images,
                               pl_module.global_step, pl_module.current_epoch, batch_idx, sample_tokens)
            else:   # for ldm
                self.log_local_rgb(split, images,
                                   pl_module.global_step, pl_module.current_epoch, batch_idx, sample_tokens)
